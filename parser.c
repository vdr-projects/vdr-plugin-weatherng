/*
 *  $Id: parser.c,v 1.2 2005/01/18 08:46:02 boonkerz Exp $
 */
...
bool xStartElem(const std::string &name, std::map<std::string, std::string> &attrs) {
	...
	if(name=="day"){
		count++;
		ATTRIB_MAN_STRING("t",day);
		ATTRIB_MAN_STRING("dt",date);
	}
...
bool xEndElem(const std::string &name) {
	if (context[context.size()-1]=="part"){
		locked= false;
	}
	if (context.size()>0){
		context.pop_back();   
...
void xmlParse(std::string code) {
	...
	if (!Get(url.c_str(),DestinationDir + "/daten.dat")){
